...
<script type="text/javascript">
	function showAddPage() {
		location.href = "${pageContext.request.contextPath}/video/insertVideo";
	}
		...
		Confirm.show('溫馨提示', '您確定要刪除' + name + '嗎？', {
			'Delete' : {
				...
				'callback' : function() {
					...
					$.post("${pageContext.request.contextPath}/video/delVideoById", params, function(data) {
						if (data == 'success') {
				...
				<tbody>
					<c:forEach items="${page.rows}" var="video" varStatus="status">
						<tr>
							...
							<td>${video.time}</td>
							<td>${video.playNum}</td>
							<td><a href="${pageContext.request.contextPath}/video/updateVideo?id=${video.id}"><span
									class="glyphicon glyphicon glyphicon-edit" aria-hidden="true"></span></a></td>
							<%--<td><a href="${pageContext.request.contextPath}/video/delVideoById?id=${video.id}"><span--%>
									<%--class="glyphicon glyphicon glyphicon-trash" aria-hidden="true"></span></a></td>--%>
